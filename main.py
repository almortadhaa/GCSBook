...
# 2. البيانات الأساسية (تُجلب من إعدادات Render)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await context.bot.send_message(chat_id=ADMIN_ID, text=alert, parse_mode='MarkdownV2')
    except: pass
...
async def process_verify_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def track_subscribers(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if result.new_chat_member.status == "member":
        await log_to_sheet(result.new_chat_member.user, "منضم للقناة")
