...
# 3. إعداد قائمة الأوامر (تظهر بجانب مربع النص)
...
# مراقبة انضمام أعضاء جدد
async def track_subscribers(update: Update, context: ContextTypes.DEFAULT_TYPE):
    result = update.chat_member
    if result.new_chat_member.status == "member":
        user = result.new_chat_member.user
        await log_to_sheet(user, "منضم للقناة")
        try:
            alert = f"🔔 *عضو جديد انضم للقناة*\n━━━━━━━━━━━━━━\nالاسم: {user.full_name}\nالآيدي: `{user.id}`"
            await context.bot.send_message(chat_id=MY_ADMIN_ID, text=alert, parse_mode='Markdown')
        except: pass
...
# دالة التجاهل للمنشورات العشوائية
...
async def main():
    ...
    app.add_handler(ChatMemberHandler(track_subscribers, ChatMemberHandler.CHAT_MEMBER))
    
    # الفلتر: يتجاهل أي رسالة نصية لا تبدأ بأمر ولا تقع ضمن المحادثة
