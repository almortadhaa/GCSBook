...
async def process_phone(update, context):
    ...
    if phone == str(row_data[1]).strip():
        ...
        # --------- بداية الإضافة ---------
        # حفظ بيانات التقرير في user_data لإعادة استخدامها عند تصدير PDF
        ...
        await update.message.reply_text(
            "هل ترغب في تصدير نتيجة الاستعلام إلى ملف PDF أو بدء استعلام جديد؟",
...
async def handle_pdf_and_restart_buttons(update, context):
    ...
    if data == 'export_pdf':
        ...
        pdf = FPDF()
        pdf.add_page()
        # pdf.add_font("Arial", "", r"arial.ttf", uni=True) if hasattr(pdf, "add_font") else None  # تم التعطيل حتى لا يسبب خطأ
        pdf.set_font("helvetica", size=14)  # استخدام helvetica الآمن دائمًا
        pdf.set_rtl(True)
    ...
    # زر بدء الاستعلام الجديد
        ...
        # إعادة توجيه المستخدم للبداية
        keyboard = [[InlineKeyboardButton("نعم أعرفه ✅", callback_data='know'), InlineKeyboardButton("لا أعرفه ❌", callback_data='dont_know')]]
        await query.message.reply_text(
            "أهلاً بك من جديد، هل تعرف رقمك الوظيفي؟",
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode='MarkdownV2'
        )
        # حذف بيانات PDF السابقة من user_data
...
async def main():
    ...
    # بداية الإضافة: هاندلر لزر تصدير PDF وزر بدء استعلام جديد
