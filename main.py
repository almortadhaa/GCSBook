...
async def handle_choice(update, context):
    ...
    if query.data == 'know':
        ...
        await query.edit_message_text("أدخل رقم التحقق (أول 4 أرقام من هاتفك + آخر 4 أرقام من بطاقتك):")
        return ASKING_VERIFY
    return CHOOSING

async def process_verify(update, context):
    verify_code = update.message.text.strip()
    # التحقق في Sheet02 (العمود الأول)
    cell = SHEET_VERIFY.find(verify_code, in_column=1)
    if cell:
        # افتراض أن العمود الثاني هو الرقم الوظيفي والثالث هو الاسم
        job_id = SHEET_VERIFY.cell(cell.row, 2).value
        name = SHEET_VERIFY.cell(cell.row, 3).value
        await update.message.reply_text(f"✅ تم التحقق بنجاح!\nالرقم الوظيفي: `{job_id}`\nالاسم: `{name}`\n\nاضغط /start للبدء من جديد.", parse_mode='Markdown')
...
async def process_id(update, context):
    ...
    if cell:
        ...
        await update.message.reply_text("✅ تم العثور عليه. أرسل رقم هاتفك للتحقق:")
        return ASKING_PHONE
