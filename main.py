...
async def handle_choice(update, context):
    ...
    await query.answer()
    if query.data == 'know':
        await query.edit_message_text("أدخل رقمك الوظيفي الآن \(باللغة الإنجليزية\):", reply_markup=NUMERIC_KEYBOARD)
        return ASKING_ID
    elif query.data == 'dont_know':
        await query.edit_message_text("أدخل رقم التحقق المكون من \(أول اربعة ارقام من هاتفك \+ آخر اربعة ارقام من بطاقتك\):", reply_markup=NUMERIC_KEYBOARD)
