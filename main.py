...
async def start(update, context):
    ...
    await update.message.reply_text("أهلاً بك عزيزي الموظف.. هل تعرف رقمك الوظيفي؟", reply_markup=InlineKeyboardMarkup(keyboard))
    return CHOOSING
...
async def handle_choice(update, context):
    ...
    await query.answer()
    if query.data == 'know':
        await query.edit_message_text("أدخل رقمك الوظيفي الآن \(باللغة الإنجليزية\):")
        return ASKING_ID
    elif query.data == 'dont_know':
        await query.edit_message_text("أدخل رقم التحقق المكون من (أول اربعة ارقام من رقم هاتفك + آخر اربعة ارقام من بطاقتك):")
...
async def process_verify(update, context):
    ...
    if cell:
        ...
        await update.message.reply_text(f"✅ تم التحقق!\nرقمك الوظيفي هو: `{job_id}`\nالاسم: `{name}`", parse_mode='Markdown', reply_markup=KEYBOARD_LAYOUT)
        return ConversationHandler.END
    await update.message.reply_text("❌ رقم التحقق الذي ادخلته غير مطابق، حاول مجدداً (باللغة الإنجليزية\):")
    return ASKING_VERIFY
...
async def process_id(update, context):
    ...
    if cell:
        ...
        await update.message.reply_text("✅ تم العثور على الرقم. أرسل رقم هاتفك للتحقق (باللغة الإنجليزية\):")
        return ASKING_PHONE
    await update.message.reply_text("❌ الرقم غير موجود. حاول مجدداً (باللغة الإنجليزية\):")
    return ASKING_ID
...
async def process_phone(update, context):
    ...
    if phone == str(SHEET_MAIN.cell(row, 2).value).strip():
        ...
        return ConversationHandler.END
    await update.message.reply_text("❌ رقم الهاتف غير مطابق .حاول مجدداً (باللغة الإنجليزية\):")
    return ASKING_PHONE
...
async def main():
    ...
    await app.start()
    # استخدام drop_pending_updates=True يضمن عدم حدوث تعارض عند إعادة التشغيل
