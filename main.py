...
# دالة الاتصال المحدثة
def الحصول_على_الجداول():
    try:
        # قراءة الاعتمادات من متغير البيئة
...
# دالة لتشغيل البوت في الخلفية عند بدء التطبيق
def start_bot():
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    loop.run_until_complete(bot_app.initialize())
    loop.run_until_complete(bot_app.post_init())
    loop.run_until_complete(bot_app.start())
    loop.run_until_complete(bot_app.updater.start_polling())

if __name__ == '__main__':
    # تشغيل البوت في مسار منفصل ليعمل Flask بجانبه
    start_bot()
