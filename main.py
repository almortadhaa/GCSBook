...
# لوحة مفاتيح ثابتة للردود الأساسية
...
async def help_command(update, context):
    text = (
        ...
        "2️⃣ *ثانياً:* إذا كانت إجابتك \(لا أعرفه\): سيطلب منك البوت رقم التحقق\.\n\n"
...
async def handle_choice(update, context):
    ...
    await query.answer()
    # إرسال رسالة جديدة لتجنب مشاكل التعديل على النص الأصلي
    if query.data == 'know':
        await query.message.reply_text("أدخل رقمك الوظيفي الآن \(باللغة الإنجليزية\):", reply_markup=NUMERIC_KEYBOARD)
        return ASKING_ID
    elif query.data == 'dont_know':
        await query.message.reply_text("أدخل رقم التحقق المكون من \(أول أربعة أرقام من هاتفك \+ آخر أربعة أرقام من بطاقتك\):", reply_markup=NUMERIC_KEYBOARD)
...
async def process_verify(update, context):
    ...
    await update.message.reply_text("❌ رقم التحقق غير مطابق، حاول مجدداً:", reply_markup=NUMERIC_KEYBOARD)
    return ASKING_VERIFY
...
async def process_id(update, context):
    ...
    if cell:
        ...
        await update.message.reply_text("✅ تم العثور على الرقم\. أرسل رقم هاتفك للتحقق:", reply_markup=NUMERIC_KEYBOARD)
        return ASKING_PHONE
    await update.message.reply_text("❌ الرقم غير موجود\. حاول مجدداً:", reply_markup=NUMERIC_KEYBOARD)
    return ASKING_ID
...
async def process_phone(update, context):
    ...
    await update.message.reply_text("❌ رقم الهاتف غير مطابق\. حاول مجدداً:", reply_markup=NUMERIC_KEYBOARD)
    return ASKING_PHONE
