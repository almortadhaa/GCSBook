...
from telegram import InlineKeyboardButton, InlineKeyboardMarkup, ReplyKeyboardMarkup, BotCommand, ForceReply
...
async def handle_choice(update, context):
    ...
    await query.answer()
    if query.data == 'know':
        await query.message.reply_text("أدخل رقمك الوظيفي الآن \(باللغة الإنجليزية\):", parse_mode='MarkdownV2', reply_markup=ForceReply(selective=True))
        return ASKING_ID
    elif query.data == 'dont_know':
        await query.message.reply_text("أدخل رقم التحقق المكون من \(أول اربعة ارقام من رقم هاتفك \+ آخر اربعة ارقام من بطاقتك\):", parse_mode='MarkdownV2', reply_markup=ForceReply(selective=True))
...
async def process_verify(update, context):
    ...
    await update.message.reply_text("❌ رقم التحقق الذي ادخلته غير مطابق، حاول مجدداً \(باللغة الإنجليزية\):", parse_mode='MarkdownV2', reply_markup=ForceReply(selective=True))
    return ASKING_VERIFY
...
async def process_id(update, context):
    ...
    if cell:
        ...
        await update.message.reply_text("✅ تم العثور على الرقم\. أرسل رقم هاتفك للتحقق \(باللغة الإنجليزية\):", parse_mode='MarkdownV2', reply_markup=ForceReply(selective=True))
        return ASKING_PHONE
    await update.message.reply_text("❌ الرقم غير موجود\. حاول مجدداً \(باللغة الإنجليزية\):", parse_mode='MarkdownV2', reply_markup=ForceReply(selective=True))
    return ASKING_ID
...
async def process_phone(update, context):
    ...
    await update.message.reply_text("❌ رقم الهاتف غير مطابق\. حاول مجدداً \(باللغة الإنجليزية\):", parse_mode='MarkdownV2', reply_markup=ForceReply(selective=True))
    return ASKING_PHONE
