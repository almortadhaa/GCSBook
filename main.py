...
async def start(update, context):
    ...
    await update.message.reply_text(escape_markdown("أهلاً بك عزيزي الموظف. هل تعرف رقمك الوظيفي؟"), reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='MarkdownV2')
    return CHOOSING
...
async def help_command(update, context):
    text = (
        ...
        "أولاً: الأمر /start لبدء الاستعلام، سيظهر لك خيار (هل تعرف رقمك الوظيفي؟).\n"
        "   - إذا كنت تعرفه: قم بإدخاله، فإذا كان صحيحاً سيطلب منك إدخال رقم هاتفك لتظهر بياناتك.\n\n"
        "ثانياً: إذا كانت إجابتك (لا أعرفه): سيطلب منك البوت رقم التحقق (رقم مزدوج مكون من 8 أرقام: الجزء الاول تمثل الاربعة الأرقام الاولى من رقم هاتفك والجزء الثاني تمثل الاربعة الارقام الاخيرة من بطاقتك الشخصية).\n"
...
async def handle_choice(update, context):
    ...
    await query.answer()
    if query.data == 'know':
        await query.edit_message_text(escape_markdown("أدخل رقمك الوظيفي الآن (باللغة الإنجليزية):"), parse_mode='MarkdownV2')
        return ASKING_ID
    elif query.data == 'dont_know':
        await query.edit_message_text(escape_markdown("أدخل رقم التحقق المكون من (أول اربعة ارقام من رقم هاتفك + آخر اربعة ارقام من بطاقتك):"), parse_mode='MarkdownV2')
    ...
    if cell:
        ...
        await update.message.reply_text(
            escape_markdown(
                f"✅ *تم التحقق!*\n\n"
                f"👤 الاسم: `{job_id}`\n"
                f"🆔 رقمك الوظيفي هو: `{name}`\n\n"
                f"⚠️ *ملاحظة:* احفظ رقمك الوظيفي واضغط /start مرة أخرى لمواصلة الاستعلام عن مستحقاتك."
            ),
            parse_mode='MarkdownV2', 
            reply_markup=KEYBOARD_LAYOUT
        )
        return ConversationHandler.END
    await update.message.reply_text(escape_markdown("❌ رقم التحقق الذي ادخلته غير مطابق. حاول مجدداً (باللغة الإنجليزية):"), parse_mode='MarkdownV2')
    return ASKING_VERIFY
...
async def process_id(update, context):
    ...
    if cell:
        ...
        await update.message.reply_text(escape_markdown("✅ تم العثور على الرقم. أرسل رقم هاتفك للتحقق (باللغة الإنجليزية):"), parse_mode='MarkdownV2')
        return ASKING_PHONE
    await update.message.reply_text(escape_markdown("❌ الرقم غير موجود. حاول مجدداً (باللغة الإنجليزية):"), parse_mode='MarkdownV2')
    return ASKING_ID
...
async def process_phone(update, context):
    ...
    if phone == str(row_data[1]).strip():
        ...
        # --------- بداية الإضافة ---------
        context.user_data['pdf_result'] = [f"{h}: {v}" for h, v in zip(headers, row_data)]
        pdf_export_kb = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("📄 تصدير إلى PDF", callback_data='export_pdf'),
                InlineKeyboardButton("🔄 بدء استعلام جديد", callback_data='start_new'),
            ]
        ])
        await update.message.reply_text(
            escape_markdown("هل ترغب في تصدير نتيجة الاستعلام إلى ملف PDF أو بدء استعلام جديد؟"),
            reply_markup=pdf_export_kb,
            parse_mode='MarkdownV2'
        )
        # --------- نهاية الإضافة ---------
        return ConversationHandler.END

    await update.message.reply_text(escape_markdown("❌ رقم الهاتف غير مطابق. حاول مجدداً (باللغة الإنجليزية):"), parse_mode='MarkdownV2')
    return ASKING_PHONE

# ----- بداية الإضافة: فنكشن لتصدير النتائج PDF -----
async def handle_pdf_and_restart_buttons(update, context):
    query = update.callback_query
    data = query.data

    # زر تصدير PDF
    if data == 'export_pdf':
        result = context.user_data.get('pdf_result', [])
        if not result:
            await query.answer("لا توجد نتيجة حالية للتصدير.")
            return
        ...
        pdf_file_name = f"result_{query.from_user.id}_{datetime.now().strftime('%Y%m%d%H%M%S')}.pdf"
        pdf.output(pdf_file_name)

        with open(pdf_file_name, "rb") as f:
            await query.message.reply_document(
                document=f, 
                filename="نتيجة_الاستعلام.pdf",
                caption="📄 تم تصدير النتائج إلى ملف PDF بنجاح!"
            )
        await query.answer("تم تصدير النتيجة بنجاح!")
        start_kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔄 بدء استعلام جديد", callback_data='start_new')]])
        await query.message.reply_text(
            escape_markdown("يمكنك البدء باستعلام جديد بالضغط أدناه:"),
            reply_markup=start_kb,
            parse_mode='MarkdownV2'
        )

    # زر بدء الاستعلام الجديد
    elif data == 'start_new':
        await query.answer("يمكنك البدء باستعلام جديد.")
        keyboard = [[InlineKeyboardButton("نعم أعرفه ✅", callback_data='know'), InlineKeyboardButton("لا أعرفه ❌", callback_data='dont_know')]]
        await query.message.reply_text(
            escape_markdown("أهلاً بك من جديد، هل تعرف رقمك الوظيفي؟"),
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode='MarkdownV2'
        )
        context.user_data.pop('pdf_result', None)
        return CHOOSING
...
async def main():
    ...
    app.add_handler(CallbackQueryHandler(handle_pdf_and_restart_buttons, pattern='^(export_pdf|start_new)$'))
